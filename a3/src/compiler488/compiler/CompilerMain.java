...
public class CompilerMain
{
	...
	private static void analyzeCommandLine(CommandLine cl)
	{
		...
		if (cl.hasOption(testSuiteOpt))
		{
			...
			// TODO: parse sub-command-line for test-run
	...
	private static Group getCommandLineOptions()
	{
		...
		outFileFormat = defaultOpt
				.withDescription("FORMAT=<488|CPP|X86> defaults to 488, which outputs equivalent, compilable 488 code. Another valid format is" +
	...
	/*
	 * TODO: Missing part from old Main.java (for code generation assignment
	 * only)... // Initialize Machine before code generation try{
	 * Machine.powerOn(); } catch( Exception e) {
	 * System.err.println("Exception during Machine initialization");
	 * System.err.println(e.getClass().getName() + ": " + e.getMessage());
	 * System.exit( 140 ); }
	 * 
	 * try{ CodeGen codeGen = new CodeGen(programAST); codeGen.run(); } catch(
	 * Exception e) { System.err.println("Exception during Code Generation");
	 * System.err.println(e.getClass().getName() + ": " + e.getMessage());
	 * e.printStackTrace (); errorOccurred = true ; }
	 * 
	 * if( errorOccurred ){
	 * System.out.println("Processing Terminated due to errors"); return ; }
	 * else System.out.println("End of Compilation");
	 * 
	 * if( supressExecution ) {
	 ...
	 * 
	 * setTraceStream( executeTraceFileName ); setInputSource(
	 * executeInputFileName );
	 * 
	 * dumpStream = System.out ; try{ Machine.run( ) ; } catch(
	 * ExecutionException e) {
	 * System.err.println("Exception during Machine Execution" +
	 * e.getMessage()); // Run error has already dumped machine state. return ;
	 * } catch( Exception e) {
	 * System.err.println("Unexpected Exception during Machine Execution");
	 * System
	 * .err.println("Please file a Bug Report with the course instructor");
	 * System.err.println(e.getClass().getName() + ": " + e.getMessage());
	 * e.printStackTrace (); System.exit( 200 ); }
	 * 
	 * if( traceStream != null && traceStream != saveSysOut )
