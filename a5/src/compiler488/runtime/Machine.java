 ...
 * The class writes to System.out and System.err and reads from System.in Those
 * streams may be redirected by the calling program
 * 
 * @author Dave Wortman
...
public final class Machine
{
	 ...
	 * this routine should be called before code generation and after Main has
	 * processed command line arguments. Initializes display and stack memory.
	 */
	 ...
	 * Does nothing if toPad is already longer than toSize Assumes value of
	 * toSize is less than length of Blanks
	 * 
	 * @param toPad
	 ...
	 */
	private final static void dumpStack(StringBuffer sink, int howMany)
	{
	 ...
	 * Dumps key machine registers, active part of display <B> top 8 (or less)
	 * entries in the hardware stack
	 * 
	 * @param msg
	 ...
	 */
	private final static void dumpMachineState(String msg, int pc, int msp, int mlp)
	{
		StringBuffer S = new StringBuffer("\t");
		print("\n" + msg);
		// dump control of registers
		print("  pc = " + pc + ", msp = " + msp + ", mlp = " + mlp + "\n");

		// Dump the active display
		if (displayMax >= 0)
		{
			print("\tdisplay[0 .. " + displayMax + "] = ");
	...
	/**
	 * Function for validating execution This function checks that lowBound &lt=
	 * value &lt= highBound runError is called with msg if this is not true
	 * 
	 * @param value
	 ...
	 * Changes the state of the machine ie: the private instance arrays 'memory'
	 * and 'display', and the private instance variables 'pc', and 'msp'.
	 * 
	 * @throws ExecutionException
	 */
			...
			throws ExecutionException
	{
		...
		boolean tracing = false;// Main.traceExecution ;
		...
		while (executing)
		{
			...
			case EQ:
				spop();
				memory[msp - 1] = (short) (memory[msp - 1] == topp1()
			...
			case OR:
				...
				rangeCheck(memory[msp], MACHINE_FALSE, MACHINE_TRUE,
						"OR operand is not a Boolean value");
				memory[msp - 1] = (short) ((memory[msp - 1] == MACHINE_TRUE
			 ...
			 */
			case PRINTC:
				CompilerMain.onMachineOutput(Character.toString((char) (memory[msp - 1] & CHARMASK)));
				spop();
				break;
			 ...
			 */
			case PRINTI:
				CompilerMain.onMachineOutput(Integer.toString(memory[msp - 1]));
				spop();
				break;
